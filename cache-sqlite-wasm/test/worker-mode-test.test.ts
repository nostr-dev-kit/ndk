...
describe("Worker mode test", () => {
    ...
    beforeEach(async () => {
        cache = new NDKCacheAdapterSqliteWasm({
            dbName: "test-worker-mode",
            useWorker: true,
            workerUrl: new URL("../dist/worker.js", import.meta.url).toString(),
        });
        ndk = new NDK({ cacheAdapter: cache });
        await cache.initializeAsync(ndk);
    });

    it("should work with worker mode", async () => {
        ...
        console.log("Worker mode stats:", stats);
