...
By default, [NDK subscriptions](/core/docs/fundamentals/subscribing.md) use cross-subscription matching: when an event
comes in from any relay, it's delivered to
...
Cached events are checked against their known relay provenance. If a cached event was previously seen on a relay in your
exclusive relaySet, it will be delivered:
...
The `exclusiveRelay` check happens after filter matching, so there's minimal performance impact. The check only applies
to subscriptions that have both:
