...
export function loadCashuMintInfo(
    db: Database,
    mintUrl: string,
    maxAgeInSecs?: number,
): CashuMintInfo | undefined {
    const now = Math.floor(Date.now() / 1000);

    const stmt = db.prepare(
        "SELECT info, cached_at FROM cashu_mint_info WHERE mint_url = ?",
    );
    stmt.bind([mintUrl]);
    ...
    if (stmt.step()) {
        ...
        if (maxAgeInSecs && now - cachedAt > maxAgeInSecs) {
            console.debug(`🗄️ [Cache] Mint info for ${mintUrl} expired (age: ${now - cachedAt}s, max: ${maxAgeInSecs}s)`);
            stmt.free();
            return undefined;
        }
