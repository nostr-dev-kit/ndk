import type { NDKSigner, NDKSignerStatic } from "./index.js";

/**
 * Registry to hold signer types and their corresponding deserialization functions.
 * Signer packages (like ndk-mobile for NIP-55) can add their types here.
 */
export const signerRegistry: Map<string, NDKSignerStatic<NDKSigner>> = new Map();

/**
 * Register a signer in the registry
 */
export function registerSigner(type: string, signerClass: NDKSignerStatic<NDKSigner>): void {
  signerRegistry.set(type, signerClass);
}

/**
 * Initialize the registry with all built-in signers
 * This function is called after all signers are defined to avoid circular dependencies
 */
export function initializeSignerRegistry(): void {
  // Import here to avoid circular dependencies
  const { NDKPrivateKeySigner } = require("./private-key/index.js");
  const { NDKNip07Signer } = require("./nip07/index.js");
  const { NDKNip46Signer } = require("./nip46/index.js");

  // Register built-in signers
  registerSigner("private-key", NDKPrivateKeySigner);
  registerSigner("nip07", NDKNip07Signer);
  registerSigner("nip46", NDKNip46Signer);
}

