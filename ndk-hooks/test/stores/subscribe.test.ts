...
describe('Subscribe Store - Advanced Tests', () => {
    ...
    describe('filterMutedEvents', () => {
        ...
        const eventMutedPubkey = new MockEvent({
            id: 'ev-pubkey',
            pubkey: mutedPubkey,
        }) as unknown as NDKEvent;
        const eventMutedWord = new MockEvent({
            id: 'ev-word',
            pubkey: nonMutedPubkey,
            content: `Contains ${mutedWord}`,
        }) as unknown as NDKEvent;
        const eventMutedTag = new MockEvent({
            id: 'ev-tag',
            pubkey: nonMutedPubkey,
            tags: [['t', mutedHashtag]],
        }) as unknown as NDKEvent;
        const eventMutedRef = new MockEvent({
            id: 'ev-ref',
            pubkey: nonMutedPubkey,
            tags: [['e', mutedEventId]],
        }) as unknown as NDKEvent;
        const eventMutedOwnId = new MockEvent({
            id: mutedEventId,
            pubkey: nonMutedPubkey,
        }) as unknown as NDKEvent;
        const eventNonMuted = new MockEvent({
            id: 'ev-ok',
            pubkey: nonMutedPubkey,
        ...
        it('should filter based on mutedHashtags', () => {
            // Removed (case-insensitive) as hook handles lowercasing
            const criteria: MuteCriteria = {
                mutedPubkeys: new Set(),
                mutedEventIds: new Set(),
