...
<div class="container">
  ...
  {:else}
      ...
      {#if sessions.current}
        <div class="info-box">
          <div class="info-row">
            ...
            <span class="info-value" data-testid="current-pubkey">{sessions.current.pubkey}</span>
          </div>
          <div class="info-row">
            <span class="info-label">Profile Name:</span>
            <span class="info-value" data-testid="profile-name">
              {sessions.profile?.name || sessions.profile?.display_name || sessions.profile?.displayName || '(no name set)'}
            </span>
          </div>
          <div class="info-row">
            <span class="info-label">About:</span>
            <span class="info-value">{sessions.profile?.about || '(no bio)'}</span>
          </div>
          <div class="info-row">
            <span class="info-label">Follows:</span>
            <span class="info-value" data-testid="follows-count">{sessions.follows.size} pubkeys</span>
          </div>
          <div class="info-row">
            <span class="info-label">Mutes:</span>
            <span class="info-value">{sessions.mutes.size} pubkeys</span>
          </div>
        </div>

        <!-- Follows List -->
        {#if sessions.follows.size > 0}
          <h3 style="margin-top: 20px;">Following:</h3>
          <div class="follows-list" data-testid="follows-list">
            {#each Array.from(sessions.follows) as followPubkey}
    ...
    <!-- All Sessions -->
    <div class="section">
      <h2>All Sessions ({sessions.all.length})</h2>
