...
export async function checkTokenProofs(
    this: NDKCashuWallet,
...
export async function checkTokenProofsForMint(
    ...
    d("checking %d proofs in %d tokens for spent proofs for mint %s", allProofs.length, tokens.length, mint);
    ...
    for (const token of tokens) {
        ...
        if (dirty) {
            ...
            console.log("👉 token has spent proofs", spentProofs.map(p => p.id), token.rawEvent(), token.onRelays, token.relay);
        }
    }
