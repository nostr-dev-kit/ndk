...
import { NDKNutzap } from "../../cashu/nutzap";
...
class NutzapHandler {
    ...
    private async processRedeemQueue() {
        ...
        for (const [mint, proofs] of Object.entries(mintProofs)) {
            ...
            wallet.checkProofsSpent(proofs)
                .then(async (spentProofs) => {
                    ...
                    for (const nutzap of this.redeemQueue.values()) {
                        if (nutzap.mint === mint) {
                            ...
                            const validProofs = nutzapProofs.filter(p => !spentProofSecrets.includes(p.secret));
    ...
    private async redeem(event: NDKEvent) {
        ...
        try {
            ...
            if (!wallet) {
                ...
                throw new Error("wallet not found for nutzap (p2pk: " + p2pkPubkey + ") "+nutzap.content);
            }
