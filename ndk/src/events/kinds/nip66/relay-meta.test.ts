...
const setupNDKEvents = (ndk: NDK) => {
  NDKEVENT_30066_1 = new NDKRelayMeta(ndk, EVENT_30066_A);
};

describe('NDKRelayMeta', () => {
  let relayMeta: NDKRelayMeta;
  const ndk = new NDK();
  setupNDKEvents(ndk);

  beforeEach(() => {
    relayMeta = new NDKRelayMeta(ndk, EVENT_30066_A);
  });
  ...
  describe('@static', () => {
    describe('from', () => {
      it('should return a NDKRelayMeta instance', () => {
        const newRelayMeta = NDKRelayMeta.from(NDKEVENT_30066_1);
        expect(newRelayMeta).toBeInstanceOf(NDKRelayMeta);
