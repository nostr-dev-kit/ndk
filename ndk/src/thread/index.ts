...
export function eventsBySameAuthor(
    op: NDKEvent,
    events: NDKEvent[]
) {
    const eventsByAuthor = new Map<NDKEventId, NDKEvent>();
    eventsByAuthor.set(op.id, op);
    events.forEach(event => { if (event.pubkey === op.pubkey) { eventsByAuthor.set(event.id, event); } });
...
export function getEventReplyIds(event: NDKEvent): NDKEventId[] {
    return event.getMatchingTags("e")
