...
import { NDKZapConfirmation, NDKZapPaymentDetails, NDKZapper, NutPaymentInfo } from ".";
...
jest.mock("./ln.js", () => ({
    getNip57ZapSpecFromLud: jest.fn(async () => {
        return {"status":"OK","tag":"payRequest","callback":"https://primal.net/lnurlp/pablof7z/callback","metadata":"[[\"text/plain\",\"sats for pablof7z@primal.net\"]]","minSendable":1000,"maxSendable":11000000000,"nostrPubkey":"f81611363554b64306467234d7396ec88455707633f54738f6c4683535098cd3","allowsNostr":true,"commentAllowed":200};
    }),
}));
...
describe("NDKZapper", () => {
    describe("getZapSplits", () => {
        ...
        it("uses the author pubkey when the target is the user", () => {
            const user = ndk.getUser({ pubkey: "fa984bd7dbb282f07e16e7ae87b26a2a7b9b90b7246a44771f0cf5ae58018f52" });
...
describe("getZapMethod", () => {
    ...
    it('identifies when the user has signaled nutzaps', async () => {
        ...
        expect((zapMethod.data as NutPaymentInfo).mints).toEqual(["https://mint1", "https://mint2"]);
    })
    ...
    it('defaults to nip57 when the user has not signaled nutzaps', async () => {
        ...
        const zapper = new NDKZapper(user, 1000);
        zapper.onNutPay = async (payment: NDKZapPaymentDetails): Promise<false> => false;
        zapper.onLnPay = async (payment: NDKZapPaymentDetails): Promise<NDKZapConfirmation> => false;
        const zapMethod = await zapper.getZapMethod(ndk, "fa984bd7dbb282f07e16e7ae87b26a2a7b9b90b7246a44771f0cf5ae58018f52");
