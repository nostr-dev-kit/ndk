...
<div
	data-generic-embedded=""
	data-variant={variant}
	class="border border-border rounded-lg bg-card {variant === 'compact' ? 'p-2' : 'p-3'} {variant === 'inline' ? 'max-w-[500px]' : ''}"
>
	<EventCard.Root {ndk} {event}>
		...
		{#if altTag}
			<div class="alt-tag-container {variant === 'compact' ? 'p-2 mb-2' : 'p-3 mb-3'}">
				<div class="break-all text-foreground font-medium leading-relaxed {variant === 'compact' ? 'text-[0.8125rem]' : 'text-[0.9375rem]'}">
					{altTag}
				</div>
			</div>
		{/if}
