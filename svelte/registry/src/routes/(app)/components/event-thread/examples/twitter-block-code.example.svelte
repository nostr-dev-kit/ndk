  ...
  <div class="thread-container">
    <div class="thread">
      ...
      {#each thread.events as node, i (node.id)}
        {#if node.event}
          <EventCard.Root {ndk} event={node.event} class="tweet-event-card !p-0 !rounded-none !shadow-none !bg-transparent">
            <div
              class="tweet"
      ...
      {#if thread.replies.length > 0}
        {#each thread.replies as reply (reply.id)}
          <EventCard.Root {ndk} event={reply} class="tweet-event-card !p-0 !rounded-none !shadow-none !bg-transparent">
            <div class="tweet">
              <div class="timeline">
                <EventCard.Avatar class="avatar" />
              </div>
...
<style>
  ...
  .tweet-event-card :global(.event-card-header) {
