<!--
  ...
  3. index.txt = Simplified documentation version with these changes:
     ...
     - Use inline prop destructuring: let { ndk, event, renderer } = $props();
...
<script lang="ts">
  ...
  import type { ContentRenderer } from '$lib/registry/ui/content-renderer';
  ...
  let { ndk, event, renderer }: Props = $props();
</script>
