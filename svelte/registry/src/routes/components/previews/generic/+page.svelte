<script lang="ts">
  ...
  import GenericCompactExample from './examples/generic-compact.svelte';
  ...
  <section class="mb-12">
    <div class="p-6 border border-border rounded-lg bg-card">
      ...
      <p class="text-sm text-muted-foreground mb-4">
        When an event kind has no registered handler, GenericEmbedded provides an intelligent fallback
        that leverages NIP-31 and NIP-89 to help users understand and interact with unknown event kinds.
      </p>
      <div class="text-sm space-y-2">
        <div><strong>Handles:</strong> Any kind not in the registry</div>
        <div><strong>Component:</strong> <code class="px-2 py-1 bg-muted rounded">GenericEmbedded</code></div>
        <div><strong>NIP-31:</strong> Displays human-readable alt tag summaries</div>
        <div><strong>NIP-89:</strong> Discovers and lists compatible apps that can handle the event</div>
        <div><strong>Features:</strong> One-click links to open events in compatible apps (web, iOS, Android)</div>
      </div>
    </div>
  </section>
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="p-3 bg-muted rounded text-xs font-mono space-y-1">
            ...
            <div>import {`{ defaultKindRegistry }`} from '../../registry.svelte';</div>
            <div class="h-2"></div>
            <div>defaultKindRegistry.add([9999], MyKindEmbedded);</div>
            <div class="h-2"></div>
