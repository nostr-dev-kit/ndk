...
export type MetaSubscribeSortOption =
  | "time"
  | "count"
  | "tag-time"
  | "unique-authors";

export interface MetaSubscribeConfig
  extends Omit<SubscribeConfig, "dedupeKey"> {
  /**
...
export function createMetaSubscription<T extends NDKEvent = NDKEvent>(
  ...
  $effect(() => {
    ...
    const optsChanged =
      JSON.stringify(currentNdkOpts) !== JSON.stringify(newNdkOpts);
  ...
  async function handlePointerEvents(pointerEvents: NDKEvent[]) {
    ...
    if (addresses.length > 0) {
      ...
      const byAuthor = new Map<
        string,
        { kinds: Set<number>; dTags: Set<string> }
      >();
  ...
  function updateEvents() {
    const events = Array.from(targetEventMap.values());
      ...
      case "tag-time":
        ...
        events.sort((a, b) => {
          ...
          const aMaxTime = Math.max(
            ...aPointers.map((p) => p.created_at || 0),
            0,
          );
          const bMaxTime = Math.max(
            ...bPointers.map((p) => p.created_at || 0),
            0,
          );
