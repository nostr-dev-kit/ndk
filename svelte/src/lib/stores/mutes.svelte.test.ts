...
describe("MutesProxy", () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(NDKEvent.prototype, "publishReplaceable").mockResolvedValue(new Set());
    });
    ...
    describe("mute method", () => {
        it("should throw error when no active user", async () => {
            ...
            await expect(mutesProxy.mute("pubkey3")).rejects.toThrow("No active user");
        });

        it("should throw error when no NDK instance found", async () => {
            mockUser.ndk = null;

            await expect(mutesProxy.mute("pubkey3")).rejects.toThrow("No NDK instance found");
        });
        ...
        it("should create event with kind 10000 (MuteList)", async () => {
            ...
            vi.spyOn(NDKEvent.prototype, "publishReplaceable").mockImplementation(async function(this: NDKEvent) {
                createdEvent = this;
                return new Set();
            });
    ...
    describe("unmute method", () => {
        it("should throw error when no active user", async () => {
            ...
            await expect(mutesProxy.unmute("pubkey1")).rejects.toThrow("No active user");
        });

        it("should throw error when no NDK instance found", async () => {
            mockUser.ndk = null;

            await expect(mutesProxy.unmute("pubkey1")).rejects.toThrow("No NDK instance found");
        });
