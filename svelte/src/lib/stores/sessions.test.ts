...
describe("SessionsStore", () => {
  ...
  it("should detect race condition: activePubkey set before ndk.signer", async () => {
    ...
    // Find the operations
    const activePubkeyOp = operations.find(
      (op) => op.op === "activePubkey-set",
    );
    ...
    const activePubkeyIndex = operations.findIndex(
      (op) => op.op === "activePubkey-set",
    );
    const signerIndex = operations.findIndex((op) => op.op === "signer-set");

    console.log(
      "activePubkey set at index:",
      activePubkeyIndex,
      "with signerSet:",
      activePubkeyOp?.signerSet,
    );
