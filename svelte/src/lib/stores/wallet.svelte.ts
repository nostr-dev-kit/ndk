...
export class ReactiveWalletStore {
  ...
  constructor(ndk: NDK, sessionManager: NDKSessionManager) {
    ...
    sessionManager.subscribe((state) => {
      ...
      const pubkeyChanged =
        this.#currentPubkey && this.#currentPubkey !== activePubkey;
      ...
      if (
        walletEvent.id === this.#currentWalletEventId &&
        activePubkey === this.#currentPubkey
      ) {
        return;
      }
  ...
  get needsOnboarding(): boolean {
    const wallet = this.#wallet;
    return (
      !wallet || (wallet instanceof NDKCashuWallet && wallet.mints.length === 0)
    );
  }
...
export function createReactiveWallet(
  ndk: NDK,
  sessionManager: NDKSessionManager,
): ReactiveWalletStore {
